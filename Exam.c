...
public:
    ...
    bool PP(shared_ptr<CC> p, const vector<int>& request) {
        for (size_t i = 0; i < request.size(); ++i) {
            if (request[i] < 0 || request[i] > p->KK[i]) {
                return false;
            }
        }
        ...
        vector<shared_ptr<CC>> wokeup;
            ...
            if (p->JJ == BB::BLOCKED && p->MM[0] > 0) {
                ...
                for (size_t i = 0; i < retryRequest.size(); ++i) {
                    if (p->MM[i] > 0) {
                        retryRequest[i] = min((int)i, p->MM[i]);
                    }
                }
                bool canAlloc = true;
                for (size_t i = 0; i < retryRequest.size(); ++i) {
...
int main() {
    ...
    auto p1 = make_shared<CC>(1, AA::SYSTEM,
            10, vector<int>{5, 2});
    auto p2 = make_shared<CC>(2, AA::INTERACTIVE, 8, vector<int>{3, 2});
    auto p3 = make_shared<CC>(3, AA::BATCH,
            12, vector<int>{8, 3});
